...
{
	if (linux_accept4(0, 0, 0, linux_SOCK_CLOEXEC, 0) == linux_ENOSYS)
...
{
	if (linux_accept4(3, 0, 0, linux_SOCK_CLOEXEC, 0) != linux_EBADF)
		return TEST_FAILURE;
...
{
	if (linux_accept4(1, 0, 0, linux_SOCK_CLOEXEC, 0) != linux_ENOTSOCK)
		return TEST_FAILURE;
...
static enum TestResult test_correct_usage(void)
{
	...
	linux_fd_t client = 0;
	if (linux_accept4(server, 0, 0, linux_SOCK_CLOEXEC, &client))
	{
		linux_close(server);
		return TEST_FAILURE;
	}
